...
//          _nativeChatsOverviewCallStreamCtrl.add(msgOverView);
      ...
      case methods.KSmsI:
        {
          _nativeChatMessageReceiverStreamCtrl.add(methodCall.arguments);
          try {
            final msgOverView = MessageData.fromMap(methodCall.arguments);
            print(["dart::${methods.KSmsI}", methodCall.arguments]);
            int idx = 0;
            while (idx < overviewList.length) {
              final phone = methodCall.arguments["phone"];
              if (overviewList[idx].senderNumber == phone ||
                  (phone.lenght>9 && phone.contains(overviewList[idx].senderNumber))
              ) {
                final el = overviewList.removeAt(idx);
                msgOverView.thread_id = el.thread_id;
                break;
              }
              idx++;
            }
            overviewList.insert(0, msgOverView);
            if (overviewListListener != null) {
              try {
                print("chlid");
                overviewListListener();
